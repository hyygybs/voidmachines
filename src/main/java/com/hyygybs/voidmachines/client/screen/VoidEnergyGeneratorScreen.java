...
public class VoidEnergyGeneratorScreen extends AbstractMachineScreen<VoidEnergyGeneratorMenu> {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(VoidMachines.MODID, "textures/gui/void_energy_generator_gui.png");
    private static final int PROGRESS_BAR_X = 26;
    private static final int PROGRESS_BAR_Y = 64;

    public VoidEnergyGeneratorScreen(VoidEnergyGeneratorMenu menu, Inventory inventory, Component title) {
    ...
    protected void renderCustomTooltips(GuiGraphics graphics, int mouseX, int mouseY) {
        if (isHoveringRegion(mouseX, mouseY, ENERGY_BAR_X, ENERGY_BAR_Y, ENERGY_BAR_WIDTH, ENERGY_BAR_HEIGHT)) {
            ...
            graphics.renderTooltip(this.font, Component.translatable("gui.voidmachines.progress_info", menu.getSpeedMultiplier()), mouseX, mouseY);
        }
    }
}
