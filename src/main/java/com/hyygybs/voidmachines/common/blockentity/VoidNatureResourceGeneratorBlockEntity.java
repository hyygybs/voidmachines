...
public class VoidNatureResourceGeneratorBlockEntity extends AbstractResourceGeneratorBlockEntity {
    public VoidNatureResourceGeneratorBlockEntity(BlockPos pos, BlockState state) {
        super(VMBlockEntities.VOID_NATURE_GENERATOR.get(), pos, state, VMConfig.getResourceMachineCapacity());
    }
    ...
    protected int getOperationCost() {
        return VMConfig.getNatureGeneratorCost();
    }

    @Override
    protected int getOperationInterval() {
        return VMConfig.getNatureGeneratorInterval();
    }

    @Override
    protected List<Item> getCandidateItems(ServerLevel level) {
        return GeneratorContentCache.getNatureItems(level);
    }
}
